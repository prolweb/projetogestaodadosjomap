...
function addTrophy(name) {
    localStorage['Projeto,⚔️,HTTP-404,' + name] = '';
}
...
function drawTitle() {
    ...
    drawShadowText('PROJETO 404', 10, 10);
}
...
class ClickToStart {
    ...
    render() {
        ...
        drawShadowText('INICIAR O JOGO', 60, 150);
    }
}
...
class MainMenu {
    constructor() {
        this.items = [
            new MenuItem('NOVO GAME', 60, 150, 300, 40, newGame),
    ...
    render() {
        ...
        drawShadowText('CONFIGURAÇÕES', 60, 270);
        drawShadowText('COMANDOS', 60, 450);
        ...
        drawShadowText('W A S D - MOVER', 110, 500);
        drawShadowText('BOTÃO ESQUERDO - ESPADA', 110, 540);
        drawShadowText('BOTÃO DIREITO - ZOOM', 110, 580);
        drawShadowText('ESPAÇO - PULAR', 110, 620);
        drawShadowText('DICA: USE O ZOOM + ESPADA', 110, 680);
    }
}
...
class IntroStory {
    ...
    render() {
        ...
        drawShadowText('HTTP 404 - PROJETO JOMAP 2023 ', 20, screenTarget.height - 300);
        drawShadowText('O VILÂO AMEAÇA PARALIZAR NOSSA INTERNET', 20, screenTarget.height - 250);
        drawShadowText('VOCÊ DEVE EVITAR ISSO COM O CÓDIGO 200 OK ', 20, screenTarget.height - 175);

        if (time - this.startTime > 21) {
            drawShadowText('CLICK PARA JOGAR', 20, screenTarget.height - 100);
                    }
...
class DeathScreen {
    ...
    render() {
        ...
        overlayCtx.font = "bold 40px 'Arial Black'";
        drawShadowText('FRACASSOU NESSA!', 60, 150);
        drawShadowText('CLICK PARA CONTINUE', 60, 250);
    }
}
...
class WinScreen {
    ...
    render() {
        ...
        overlayCtx.font = "bold 40px 'Arial Black'";
        drawShadowText('VOCÊ VENCEU O VILÃO 404!', 60, 150);
        drawShadowText('A INTERNET SOBREVIVE!', 60, 200);
        drawShadowText('HTTP 200 OK  (☞ﾟヮﾟ)☞', 60, 300);
        drawShadowText('TEMPO DA PLAY: ' + gameTime.toFixed(1), 60, 400);
        drawShadowText('CLICK PARA CONTINUE', 60, 500);
    }
}
